...
fn update_entity_in_sh(spatial_hash: &mut SpatialHash, entt: Entity, old_pos: Vec3, new_pos: Vec3) {
    if spatial_hash.to_grid_coords(old_pos) != spatial_hash.to_grid_coords(new_pos) {
        // Delete old
        let old_index = spatial_hash.pos_to_index(spatial_hash.to_grid_coords(old_pos)).expect("Position out of bounds"); 
        let old_cellref = spatial_hash.get_mut(old_index).expect("Index out of bounds");
...
fn push_entity_to_sh(spatial_hash: &mut SpatialHash, entt: Entity, pos: Vec3) {
    ...
    let index = spatial_hash.pos_to_index(t).expect("Position out of bounds");    
...
pub fn movable_system(
    ...
    let max = Vector3 { x: 800, y: 600, z: 1 };

    // Borrow spatial_hash as mutable only in this block to avoid conflicts
    {
        let spatial_hash = &mut *spatial_hash;

        for (soldier_entity, mut transform, sprite_size, mut soldier_component, team) in soldier.iter_mut() {
            ...
            update_entity_in_sh(spatial_hash, soldier_entity, old_position, soldier_component.pos);
